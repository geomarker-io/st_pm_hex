...
d <-
  ...
  select(all_of(c(pred_names, "pm25", "h3", "year")))
...
grf_preds <- predict(grf, select(d_train, all_of(pred_names)))
...
# round all predictions and SEs to 4 significant digits
  ...
  system(glue::glue(
    "aws s3 cp ",
    "st_pm_hex_grf_preds_{year}.qs",
    "s3://geomarker/st_pm_hex/st_pm_hex_grf_preds_{year}.qs"
  ))
}
